...
func NewJaegerExporter(opts TracingExporterOptions) (trace.Exporter, error) {
	jOpts := jaeger.Options{
		...
		CollectorEndpoint: fmt.Sprintf("http://%s/api/traces", os.Getenv("JAEGER_ENDPOINT")),
	...
	if len(os.Getenv("JAEGER_ENDPOINT")) == 0 { // nolint:staticcheck
		return nil, errors.New("Must specify either JAEGER_ENDPOINT (default: localhost:14268)")
	}
