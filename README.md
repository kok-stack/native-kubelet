...
export VKUBELET_POD_IP=192.168.212.221
...
/mnt/d/code/native-kubelet/bin/virtual-kubelet --provider-config=/mnt/d/code/native-kubelet/config.json --kubeconfig=/root/.kube/config --nodename vkubelet-mock-0 --provider native-kubelet --startup-timeout 10s --klog.v "2" --klog.logtostderr --log-level debug --trace-exporter=jaeger --trace-sample-rate=always
```