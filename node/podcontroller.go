...
// github.com/kok-stack/native-kubelet/errdefs package in order for the
...
func (pc *PodController) syncPodFromKubernetesHandler(ctx context.Context, key string) error {
	...
	if err != nil {
		...
		if err != nil && !errdefs.IsNotFound(err) && !errors.IsNotFound(err) {
			err = pkgerrors.Wrapf(err, "failed to fetch pod with key %q from provider", key)
			span.SetStatus(err)
			return err
		}
		if errdefs.IsNotFound(err) || errors.IsNotFound(err) || pod == nil {
			return nil
		}
...
func (pc *PodController) syncPodInProvider(ctx context.Context, pod *corev1.Pod, key string) (retErr error) {
	...
	// more context is here: https://github.com/kok-stack/native-kubelet/pull/760
